...
namespace basis {
    ...
    */
	...
	operator key_type() const { return key_; }	
	...
	*/
    ...
    inline std::ostream& operator<<(std::ostream& os, const shell_data &shell) {
	static const char symbol[] = "spdfghijk";
	if (shell.type) return (os << "sp");
	return (os << symbol[shell.type]);
    }
