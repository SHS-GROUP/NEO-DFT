...
#pragma omp critical
	...
	*/
	    ...
	    BOOST_AUTO(const &Cv, wf.C(wf.virtuals()));
	    ...
	    BOOST_AUTO(t3, make_matrix<column_major>(N, nv, t[3]));
	    ...
	    blas::gemm(1, t2, Cv, 0, t3);
	    blas::gemm(1, trans(Cv), t3, 0, t4);
...
double mp2::energy(Wavefunction wf, Runtime &rt) {
	...
	nwords -= max_threads*(memory[0] + memory[1] + memory[2]);
	nwords -= max_threads*(2*N*N);
    }
    ...
    size_t MAXQS = 2*basis.max().size()*basis.max().size();
    MAXQS = std::max<size_t>(MAXQS, nwords/(nb*B*max_threads));
    MAXQS = std::min<size_t>(MAXQS, N*basis.max().size());
	...
	cout << "memory (per thread):" 
	     << (MAXQS*nb*B)*sizeof(double)/1e6 << " MB"
	     << std::endl;
