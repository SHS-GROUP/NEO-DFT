...
#ifndef BOOST_NUMERIC_BINDINGS_CUBLAS_LEVEL3_GEMM_HPP
#define BOOST_NUMERIC_BINDINGS_CUBLAS_LEVEL3_GEMM_HPP
...
#include <boost/numeric/bindings/cublas/detail/cublas_option.hpp>
//// #else
...
// * float value-type.
//
template< typename Handle, typename Order, typename TransA, typename TransB >
inline void gemm( Handle handle, const Order order, const TransA transa, const TransB transb,
        const int m, const int n, const int k, const float alpha,
        const float* a, const int lda, const float* b, const int ldb,
        const float beta, float* c, const int ldc ) {
    BOOST_STATIC_ASSERT( (is_same<Order, tag::column_major>::value) );
    cublasSgemm( handle,
		 cublas::detail::cublas_option< TransA >::value,
		 cublas::detail::cublas_option< TransB >::value,
...
struct gemm_impl {
    ...
    //
    template< typename Handle, typename MatrixA, typename MatrixB, typename MatrixC >
    static result_type invoke( Handle handle, const value_type alpha, const MatrixA& a,
        ...
        detail::gemm( handle, order(), transa(), transb(),
...
gemm( Handle handle, 
     const typename bindings::value_type< MatrixA >::type alpha,
    ...
    gemm_impl< typename bindings::value_type<
            MatrixA >::type >::invoke( handle, alpha, a, b, beta, c );
