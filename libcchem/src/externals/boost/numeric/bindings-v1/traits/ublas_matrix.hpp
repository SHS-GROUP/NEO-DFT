...
namespace boost { namespace numeric { namespace bindings { namespace traits {
  ...
  struct matrix_detail_traits< boost::numeric::ublas::matrix<T, F, ArrT>, M > 
  {
    ...
    static pointer storage (matrix_type& m) {
      ...
      // std::cout << "matrix " << vector_traits<array_type>::storage (m.data())
      // 		<< std::endl;
  ...
  public:
	...
	// std::cout << "expression " << matrix_traits<m_type>::storage (mr.expression()) << std::endl;
  ...
  public:
    static pointer storage (matrix_type& mr) {
      ...
      //std::cout << ptr <<   " " << mr.start1() << " " << mr.start2() << std::endl;
