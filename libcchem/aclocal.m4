...
AS_IF([test "$CUDA_ROOT" = "/bin/false"], [
  ...
  AS_IF([test -n "$CUDA_ROOT"], [
    AC_PATH_TOOL([CUDA], [nvcc], [], [$CUDA_ROOT/bin])], [
    AC_PATH_TOOL([CUDA], [nvcc], [], [/usr/local/cuda/cuda/bin /usr/local/cuda/bin]dnl
  ...
  AS_IF([test -z "$CUDA"], [AC_MSG_WARN([*** No CUDA compiler found ***])], [
    ...
    AC_MSG_RESULT([CUDA found in $CUDA_ROOT])
    #AC_MSG_RESULT([CUDA compiler: $CUDA])
  ])
])
