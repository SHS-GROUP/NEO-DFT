...
#undef COEFFICIENT
	...
	static boost::thread_specific_ptr<Index>& data() {
	    return instance_;
	}
	typedef host::array<const ushort3> array_ref;
	Index() {
	    index host;
	    map_ = host.map();
	...
	array_ref find(rysq::Quartet<rysq::Shell> quartet) const {
	    ...
	    if (it == map_.end())
		return array_ref(device_ptr<ushort3>(),  size);
	    // std::cout << index_.data().data() << " "
	    // 	  << (*it).second << " " << size << std::endl;
	    return array_ref(index_.begin() + (*it).second, size);
	}
	...
	map_type map_;
	host::vector<ushort3> index_;
    };
    boost::thread_specific_ptr<Index> Index::instance_;

}

rysq::cuda::thread::~thread() {
...
void rysq::cuda::detail::Quartet::initialize() const {
    if (initialized_) return;

    BOOST_AUTO(const &quartet, *host_);
