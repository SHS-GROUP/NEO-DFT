...
namespace phoenix {
	...
	static boost::phoenix::scoped_environment<
	    Env, Env, typename locals<Env, Vars>::type, Map>
	environment(const Env &env,
		    const boost::phoenix::let_actor_gen<Vars,Map> &let) {
	    namespace phoenix = boost::phoenix;
	    namespace fusion = boost::fusion;
	    typedef typename locals<Env, Vars>::type locals_type;
	    typedef phoenix::detail::initialize_local<Env> initialize;
	    locals_type locals = 
		fusion::as_vector(fusion::transform(let.vars, initialize(env)));
	    return phoenix::scoped_environment<
	    Env, Env, locals_type, Map>(env, env, locals);
	}
    };
	...
	eval_(const Env &env, const F &f, const L &locals, const V &values,
	    ...
	    BOOST_AUTO(v, back(values));
	    static const size_t size = boost::fusion::result_of::size<L>::value;
	    eval_(scope::environment(env, let(k = v)), f,
	...
	void initialize(const OuterEnv &outer, boost::mpl::int_<I> index) {
	    ...
	    initialize(scope::environment(outer, let(k = cref(v))), next);
	}
	...
	bool advance(const OuterEnv &outer, boost::mpl::int_<I> index) {
	    ...
	    using namespace boost::phoenix;
	    BOOST_AUTO(const &env, scope::environment(outer, let(k = cref(v))));
